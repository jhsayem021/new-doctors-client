...
const Users = () => {
  ...
  const [deletingUser, setDeletingUser] = useState(null);
    ...
    const {data: users=[] , refetch ,isLoading} = useQuery({
        queryKey: ['users'],
        queryFn: async ()=>{
            const res = await fetch('https://new-doctors-server-jhsayem021.vercel.app/users')
    ...
    const handleMakeAdmin = id =>{
      console.log(id)
      fetch(`https://new-doctors-server-jhsayem021.vercel.app/users/admin/${id}`, {
        method: 'PUT',
        headers:{
...
console.log(user)
fetch(`https://new-doctors-server-jhsayem021.vercel.app/users/${user._id}`,{
    method:'DELETE',
    headers: {
        authorization: `bearer ${localStorage.getItem('accessToken')}`
    }
